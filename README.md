# Java Programming - Learning Journey

Welcome to my **Java Programming** repository! This repo contains the code I wrote while learning Java for the first time.

## 📚 Topics Covered

- **Basic Syntax**: Variables, Data Types, Operators, and Control Flow (if, for, while)
- **Object-Oriented Programming (OOP)**: Classes, Objects, Inheritance, Polymorphism, Abstraction, and Encapsulation
- **Exception Handling**: Try-Catch blocks and custom exceptions
- **Collections Framework**: ArrayLists, HashMaps, LinkedLists, and more
- **File Input/Output (I/O)**: Reading from and writing to files
- **Basic Algorithms**: Sorting, Searching, etc.
- **Basic GUI**: Simple Swing or JavaFX examples (if applicable)

## 🧑‍💻 Usage

To run any of the Java programs in this repo:

1. **Clone the repo**:

    ```bash
    git clone https://github.com/your-username/java-learning-journey.git
    ```

2. **Navigate into the project folder**:

    ```bash
    cd java-programming
    ```

3. **Compile and run a program** (example for `HelloWorld.java`):

    ```bash
    javac HelloWorld.java
    java HelloWorld
    ```

## 📝 Files

Each program is contained in a single `.java` file, named based on the task or concept it demonstrates.
Files prefixed with `CSE310_` were written by me during my university Java course (CSE310). The rest were written independently at home during self-practice.
